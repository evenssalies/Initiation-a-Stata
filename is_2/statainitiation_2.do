quietly	log	using "statainitiation_2smcl.smcl", replace
...
aléatoires $X_1,X_2,\ldots$, un ratio par exemple, $Y\equiv\frac{X_1}{X_2}$. 
...
valeurs pour chacune, prendre le produit des valeurs $y_i\equiv x_{1,i}x_{2,i},
...
$\Omega=\{voiture,bus,vélo\}$ (cet ensemble s'appelle aussi "univers", 
...
paramètres ; par exemple, $Y(\{voiture\})\equiv 0$, $Y(\{bus\})\equiv 1$ et 
$Y(\{vélo\})\equiv 2$ (0, 1 et 2 sont bien dans IR). $Y$ est ici discrète. On
appelle $\{\omega\in\Omega: Y(\omega)\in \mathcal{Y}\}$, que l'on note
$Y^{-1}(\mathcal{Y})$, l'image réciproque par $Y$ de l'ensemble $\mathcal{Y}$
avec $\mathcal{Y}\in\mathcal{P}(\{0,1,2\})$. Notons que $\mathcal{P}(\{0,1,2\})$
 est l'ensemble des parties de $\{0,1,2\}$, dont chaque élément est inclu dans 
 $\{0,1,2\}$. _Idem_ pour l'ensemble de départ, on considère généralement 
l'ensemble des parties de $\Omega$, $\mathcal{B}$, dont chaque élément 
$b\subset\{voiture,bus,vélo\}$. La raison est que les événements qui nous 
...
simplifie souvent en écrivant $\{Y\in\mathcal{Y}\}$ l'événement qui nous 
intéresse. Enfin, on définit la (mesure de) probablité à valeurs dans IR, 
$P(\{Y\in\mathcal{Y}\})$, ou plus simplement $P_Y(\mathcal{Y})$ ou 
$P(\mathcal{Y})$, et $P(y)$ pour un événement particulier, avec ici $y\in
\mathcal{Y}$. Alors, $P(\mathcal{Y})=\sum(y\in\mathcal{Y})p(y)$. Cette mesure 
...
(par exemple, $p(0)=1/2$, alors qu'en fait $p(0)=P(\{\omega\in\Omega : 
Y(\omega)=0\})$ ; c'est $P(\{voiture\})$ qui vaut 1/2).
***/
...
$\{0,1\}$, avec $P(Y=1)\equiv p$ et $P(Y=0)\equiv 1-P(Y=1)=1-p$. La loi 
(fonction de masse) ne s'écrit pas $P(Y=y)=yp+(1-y)(1-p),\ y\in\{0,1\}$, mais 
comme la loi d'une Binomiale de paramètres $N=1$ et $p$ (voir __2.1.2__). Ainsi 
: $$P(Y=y)=p^y(1-p)^{1-y},\ y\in\{0,1\}.$$ Avant de simuler un échantillon de 
...
$$Pr(Y_1=1,\ldots,Y_{24}=1)=[Pr(Y=1)]^{24}=(1/2)^{24}=0,00000005960464\cong 5,96
\times 10^{-8}.$$
...
distribuée binomiale, à valeurs dans ${0,1,\ldots,N}$. En notant 
$C_N^y\equiv\frac{N!}{y!(N-y)!}$, où $x!=x(x-1)(x-2)\cdots 1$, alors 

$$P(\sum Y_i=y)=C_N^y p^y (1-p)^{N-y}, y\in\{0,1,\ldots,N\},$$
...
nombre de fois qu'une classe technologique $i$ est citée. La variable $$Y_x\equiv Card\{i : X_i=x\}$$ est le nombre de classes citées $x$ fois. 
...
Supposons donc que $P(X_i=x)=\lambda_i^x e^{-\lambda_i}/x!$, $x=1,\ldots,27$. On
...
contraire lors d'un tirage ({suit une formation, n'en suit pas}, {est au
ch\^omage, n'y est pas}, etc.), et la Binomiale permet de calculer la
...
$$\frac{n!}{n_1!\cdots n_m!}p_1^{n_1}\cdots p_m^{n_m}.$$
...
suivante : $\lim_{n\rightarrow\infty}np(n)=\lambda$ est une constante qui, 
...
$$\frac{Y-Np}{\sqrt{Np(1-p)}}.$$ Le théorème central limite (TCL) porte sur la fonction de répartition. La loi, c'est-à-dire la distribution de probabilité de 
...
$\frac{\partial}{\partial Y}\frac{1}{X}=-\frac{1}{X^2}$, qui n'existe pas au 
...
$N(0;1)$, $X$ et $Y$ est $$Pr(Y/X\le t)=\frac{1}{\pi}\left(\frac{\pi}{2}+
 \arctan(t)\right).$$
...
clear			all
cap	prog drop	cauchy		// Laisser cette ligne en commentaire, sauf si
								// vous souhaitez modifier le programme cauchy.
set seed		10
program			cauchy, rclass	// l'option rclass permet de récupérer les r()
 drop _all
 set obs		10000
 gen			X=rnormal()
 gen			Y=rnormal()
 gen			Z=Y/X			// Z est Cauchy
 su				Z, d

/* 	On récupère la moyenne de Z dans MEAN, la médiane de Z dans VAR. 
	 MEAN et MEDI sont stockés dans r(MEAN), r(MEDI) afin de ne pouvoir être 
	 écrasés par d'autres programmes. MEAN et MEDI sont stockées dans des r()
	 grâce à la commande return */
 return scalar	MEAN=r(mean)
 return scalar	MEDI=r(p50)
end

/*	Simulation Monte Carlo.
	À chaque réplication, la commande simulate exécute le programme cauchy,
	 récupère les valeurs de MEAN et MEDI, et sauve les 1000 valeurs dans une 
	 base de données dans les variables MEANV et MEDIV. 

	 Remarque : si nous ne voulez pas voir les 1000 points de réplication
		s'affichier, vous pouvez inclure l'option nodots ; faites help
		simulate dans la fenêtre de commande si vous voulez plus d'infos. */ 

simulate 		MEANV=r(MEAN) MEDIV=r(MEDI), reps(1000): cauchy

twoway			(hist 	MEANV if MEANV>=-2&MEANV<=2, ///
					color(cyan%60) xlabel(-2(.25)2) bin(50) fraction) ///
				(hist MEDIV, ///
					color(orange%20) fraction), ///
				legend(order(1 "Moyenne" 2 "Médiane"))

/* Exécuter cette ligne si vous ne voulez plus simuler cauchy */
program drop	cauchy			
...
program			MYREG, eclass
 ...
 set obs		25
 gen			X=rnormal()
 gen			Y=3*X^2+1+rnormal()
 reg			Y X
end
simulate 		_b _se, seed(21041971) reps(1000): MYREG
hist			_b_X

/* Avant le programme "cauchy", on aurait pu mettre la commande 
...
markdoc "statainitiation_2smcl.smcl", export(docx) replace
